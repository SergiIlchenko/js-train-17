...
function replaceText(word, replacement, text) {
  ...
  let p = text.replace(word, replacement);
  return p;
...
function extractTextInParentheses(str) {
  ...
  let extractedTexts = Array.from(arr, (match) => match[1]);
...
function replaceWordOccurrences(str, word, newWord) {
  let regex = RegExp(word, "g");
  let p = str.replace(regex, newWord);
  return p;
...
function checkRegexMethods(regex) {
  let regexMethods = [];
  if (regex.dotAll === true) {
    regexMethods.push("dotAll");
  }
  if (regex.multiline === true) {
    regexMethods.push("multiline");
  }
  if (regex.sticky === true) {
    regexMethods.push("sticky");
  }
  return regexMethods;
